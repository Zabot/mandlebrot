...
int mandlebrot(long double real, long double imag, int maxSteps) {
  ...
  while (counter < maxSteps &&
         sqrt(carryReal * carryReal + carryImag * carryImag) < 2) {
...
void *mandlebrotThread(void *in) {
  ...
  for (i = info->iStart, y = info->yStart; y < info->yEnd;
       i += cn->iStep, y++) {
    for (r = info->rStart, x = info->xStart; x < info->xEnd;
         r += cn->rStep, x++) {
      unsigned int o = mandlebrot(r, i, cn->maxSteps);
      if (o == cn->maxSteps)
        memcpy(cn->data + (cn->xRes * y * 3) + x * 3, cn->colors, 3);
      else
        memcpy(cn->data + (cn->xRes * y * 3) + x * 3,
               cn->colors + (o % cn->colorCount + 1) * 3, 3);
    }
  }
...
int main(int argc, char **argv) {
  ...
  printf("mandlebrot(%LF, %LF) = %i\n", rCenter, iCenter,
         mandlebrot(rCenter, iCenter, maxSteps));
